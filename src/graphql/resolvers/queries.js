...
module.exports = {
  Query: {
    ...
    Users: async (_, args, ctx) => {
      try {
        ...
        sendErrorRedirect(ctx.res, 500, err, "Inside GQL 'Users' query");
      }
    },
    Events: async (_, args, ctx) => {
      try {
        ...
        sendErrorRedirect(ctx.res, 500, err, "Inside GQL 'Events' query");
      }
    },
  },
  Event: {
    User: async (obj, args, ctx) => {
      if (obj.user_id) {
        ...
        sendErrorRedirect(ctx.res, 400, new Error(`User id is required`), 'Inside GQL Event->User subquery');
      }
    },
    Comments: async (obj, args, ctx) => {
      if (obj.id) {
        const allEventComments = await comments.findAllEventComments(obj.id);
  ...
  UserEvents: {
    ...
    local: async (obj, args) => {
      if (args.mileRadius) {
        const localEvents = await events.findEventsWithinRadius(args.mileRadius, obj.latitude, obj.longitude);
        return localEvents;
      } else return [];
    },
  },
